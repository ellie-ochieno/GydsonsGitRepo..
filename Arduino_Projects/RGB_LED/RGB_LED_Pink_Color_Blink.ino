     ...
     /**
        PROGRAM TO BLINK THE RGB LED IN PINK COLOR MAKING USE OF THE 
...
int gColorStrength = 192;        //green LED brightness intesity set to 0
int bColorStrength = 203;        //blue LED brightness intesity set to 255
...
void setup() 
{
   ...
   Serial.println(" ");
   Serial.println(" Enter the ON delay-time for the blinking LED in microseconds  ");
                                   //prompt for user input for LED's ON delay time
   delayOnTime = Serial.parseInt();
                                   //reads the user input
   
   Serial.println(" ");      
   Serial.println(" Enter the OFF delay-time for the blinking LED in microseconds  ");
